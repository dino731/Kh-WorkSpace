...
public class ElOperationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 		이 영역에 데이터를 담으면 애플리케이션 전역에서 사용가능 => 공유범위가 가장 큼
		 ...
		 * 		요청 및 응답시 매번 생성되는 객체
		 * 		이 영역에 데이터를 담으면 해당 request객체를 포워딩 받는 응답 jsp에서만 쓸 수 있다.
		 * 		=> 공유범위가 요청에 대한 응답 jsp 하나뿐
		 * 
		...
		*/
		...
		request.setAttribute("student", new Person("오창정",24,"여자"));
		...
		request.setAttribute("pOne", new Person("민경민",15,"중성"));
