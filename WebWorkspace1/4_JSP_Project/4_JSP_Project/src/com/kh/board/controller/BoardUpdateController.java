	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(multi.getOriginalFileName("reUpfile") != null) {
				...
				// 기존에 첨부파일이 등록 되어있으 경우, UPDATE문이 실행될거고, 이때 file_no가 필요함
				// FILE_NO는 originFileNo라는 키값으로 넘겨줬었음.
				if(multi.getParameter("originFileNo") != null) {
					// 새로운 첨부파일이 있고, 기존에도첨부파일이 등록되어있던 케이스
			...
			// case3 : 새로운 첨부파일 o, 기존 첨부파일 x => b, refNo가 담긴 at => BOARD에는 UPDATE, ATTACHMENT에는 INSERT
