...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 단, 여러개의 첨부파일이 있을 것이기 때문에 Attachment들을 ArrayList에 담기
			// => 적어도 1개 이상은 담길것(썸네일을 필수 입력사항이었음으로)
			
			ArrayList<Attachment> list = new ArrayList();
			for(int i = 1; i<=4; i++) { // 파일의 최대 개수는 4개 이므로 최대 4회 반복
				String key ="file"+i;
			...
			if(result > 0) { // 성공시 => list.th 요청 => 사진게시판 리스트 이미지
				...
				response.sendRedirect(request.getContextPath()+"/list.th"); // 사진게시판 리스트 작업 완료 후 변경 예정
