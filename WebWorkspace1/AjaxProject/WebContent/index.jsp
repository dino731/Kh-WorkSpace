...
<body>
	<h1>Ajax 개요</h1>
	<p>
		Asynchronus JavaScript And Xml의 약자로 <br> 서버로부터 데이터를 가져와 전체 페이지를
		새로 고치지 않고 일부만 로드할 수 있게 하는 기법<br> 우리가 기존에 a태그로 요청 및 form을 통해 요청했던
		방식은 동기식 요청 방식 <br> => 응답페이지가 돌아와야 볼 수 있음 (페이지 화명 깜박거림)<br>
		비동기식 요청을 보내기 우해서는 AJAX라는 기술이 필요함 <br>
		<br> * 동기식 / 비동기식 <br> - 동기식 : 요청 처리 후 그에 해당하는 응답 페이지가 돌아와야만
		그 다음 작업 가능 <br> 만약 서버에서 호출된 결과까지의 시간이 지연되면 무조건 계속 기다려야함 (흰화면만
		보여지게됨)<br> 전체 페이지가 리로드됨(새로고침 즉, 페이지가 기본적으로 깜빡거림)<br>
		<br> - 비동기식: 현재 페이지를 그대로 유지하면서 중간중간마다 추가적인 요청을 보내줄 수 있음 <br>
		요청을 한다고해서 다른 페이지로 넘어가지 않음 (현재 페이지 그대로 )<br> 요청을 보내놓고 그에 해당하는 응답이
		돌아올때까지 현재페이지에서 다른 작업을 할 수 있음. ex. 검색어 자동 완성기능 or 아이디 중복체크 기능 등. * 비동기식
		단점 <br> - 현재 페이지에 지속적으로 리소스가 쌓인다 => 페이지가 느려질 수 있다<br> - 페이지내
		복잡도가 증가할 수 있다. => 유지보수가 힘들어짐 <br> - 요청 후 들어온 응답 데이터를 가지고 현재페이지에서
		새로운 요소를 만들어서 뿌려줘야한다. => DOM요소 생성 구문을 숙지 못할지 힘들 수가 있다.<br>

	</p>

	<h1>javascript방식을 이용한 AJAX테스트</h1>
	<h3>1. 버튼 클릭시 get 방식으로 서버에 데이터 전송 및 응답받기</h3>
	...
	<script>
		const ajaxTest1 = () => {
			...
			// 2. xhr 객체를 설정하기 -> 전송 방식(get/post), 요청을 보내는 주소 / [동기식, 비동기식] - 기본값은 비동기식
	...
	<pre>
		...
		* 주요속성
		...
		- data : 요청시 전달할 값
		- success : ajax통신 성공시 실행할 함수를 정의 (status == 200 일떄)
		- error : ajax 통신 실패시 실행할 함수를 정의 (status != 200 일때)
		- complate : ajax 통신에 실패했든 성공했든 무조건 실행할 함수를 정의
		
		*부수적인 속성
		- async : 서버와의 비동기 처리방식 설정 여부 (기본값 true)
		- contentType : request의 데이터 인코딩 방식 정의(보내는 측의 데이터 인코딩)	
		- dataType : 서버에서 response로 넘어오는 데이터의 데이터 자료형 설정. 값이 없다면 알아서 판단함★
					 xml : 트리형태의 구조
					 json : 맵형태의 구조(일반적인 데이터 구조)
					 script :  jacascript 및 일반 STRING형태의 데이터
					 html : html 태그 자체를 return하는 방식
					 text : String 데이터
		- accept : 파라미터의 타입 설정 (사용자 특화된 파라미터 타입 설정 가능)
		- beforeSend : ajax요청을 하기 전 실행되는 이벤트 함수를 정의 (데이터 가공, header 관련 설정을 함)!
		- cache : 요청 및 결과 값을 scope에서 갖고있지 않도록 하는 것 (기본값 true)
		- crossDomain : 타 도메인 호출 가능 여부 설정 (기본값 false)
		- dataFilter : response데이터를 받았을 떄 정상적인 값을 return 할 수 있도록 데이터와 데이터 타입 설정
		- global : 기본 이벤트 사용 여부 (ajaxStrart, ajaxStop) => 선처리 작업시 이용
		- password : 서버에 접속 권한이 필요한 경우 설정
		- process Data : 서버로 보내는 값에 대한 형태 설정 여부 (기본 데이터를 원하는 경우 false 설정)
		- timeout : 서버 요청시 응답 대기 시간 설정
			...
			$("#btn1").click(function(){
				$.ajax({ 
					...
					success : function(result){ // 요청 성공시 실행할 콜백함수 정의
						// result : 서블릿으로 부터 전달 받은 값(xhr.responseText와 동일)
						$("#output1").text(result);
					},
					error 	: function(){
	...
	<br> 응답 :
	<label id="output2">현재 응답 없음</label>
	...
	<script>
		...
		function test2(){
			$.ajax({
				...
				type: "post",
				//dataType : "json",   // 이런경우도 있지만 이번엔 생략하겠다
				success : function(result){
					console.log(result);
	...
	회원 아이디 :
	<input type="text" id="userId"> 회원비밀번호 :
	...
	<button id="xmlTest">xml데이터가져오기</button>
	<div id="fiction">
		<h2>소설</h2>
			...
			$("#xmlTest").click( () =>{
				$.ajax({
					...
					success:(data)=>{
		                  ...
		                  let entity=$(data).find(":root"); // 최상위 요소 가져오기
		                  console.log(entity);
						...
						})			//언니 안냥 이거 너무 폭신하다~~~ 타자소리 너무 좋다~~~~ 아무 말이나 하고싶더디ㅏ다다다다다다드ㅏ다두다다다다콩
					}
				});
			});
		});
	...
	<h2>7. Ajax를 이용한 파일 업로드</h2>
	<input type="file" id="upfile" multiple>
	<button onclick="sendFile();">파일 전송</button>
	<script>
		function sendFile(){
			//파일 전송시 FormData객체를 생성해서 FormData내부에 파일을 넣어준 후, 요청을 보내야함.
			let form = new FormData();
			//form.append("upfile",$("#upfile")[0].files[0]);
			//form.append("upfile", document.getElementById("#upfile").files[0]);
