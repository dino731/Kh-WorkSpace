...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * Get 방식으로 요청했다면 해당 doGet메소드가 호출된다.
		 * 
		 * 첫번째 매개변수인 HttpServletRequest request에는 요청시 전달된 내용들이 담김( 사용용자가 입력한 값,
		 * 요청 전송방식, 요청한 사용자의 ip등)
		 * 
		 * 두번째 매개변수인 HttpServletResponse response에는 요청 처리 후 응답을 할때 사용하는 객체
		 * 
		 * 우선, 요청을 처리하기 위해 요청시 전달된 값(사용자가 입력한 값) 들을 뽑는다.
		 * request의 parameter 영역안에 존재 => key-value세트로 담겨있다(name 속성값- value속성값)
		 * 
		 * 따라서 request의 parameter영역으로부터 전달된 데이터를 뽑는 메소드로
		 * - request.getParameter("키값") : String(그에 해당하는 value값) => 무조건 문자열 형태로
		 * 반환되기 때문에 다른 자요형으로 변경하려면 파싱해줘야함
		 * - request.getParameterValues("키값) : String[] (그에 해당하는 value값들) => 하나의 key값으로
		 * 여러개의 value값들을 전달받을 경우(체크박스) 문자열 배열형으로 반환가능
		*/
		
		String name = request.getParameter("name"); // "아인", ""(텍스트 상자가 빈경우 빈 문자열이 넘어간다)
		String gender = request.getParameter("gender"); // "M"/"F"/null => 라디오 버튼 체크 안한 경우
		int age = Integer.parseInt(request.getParameter("age")); //"20",""(값이 없는 경우, NumberFormatException 발생)
		
		// 체크 박스와 같이 복수개의 정보를 받을 때는 배영로 받아야한다.
		String[] foods = request.getParameterValues("food"); // ["한식","일식"] / null(체크한게 없다면 null이 반환)
		/*
		 * 이 뽑아낸 값들을 가지고 요청 처리해줘야함(db와 상호작용을 함)
		 * 보통의 흐름 : Service의 메소드 호출 시 뽑은 값들을 전달(Wrapping) - DAO 호출 - DB SQL문 결과 값 반환
		 * 
		 * int result = new MemberService().inserMember(m);
		 * if(result > 0 ) : 성공이라고 보고 commit, 실패시 rollback(Service가 처리)
		 * Service와 DAO단은 거의 변동 없이 View와 Controller단의 소스코드상의 변화가 많을 것이다.
		 * 
		 * 위 요청처리를 다 했다는 가정하에 사용자가 보게될 응답페이지를 꾸밀 예정.
		 * 
		 * 장점 : Java코드내에 작성하기 때문에 반복문, 조건문등 유용한 메소드 활용이 가능
		 * 
		 * 단점 : 복잡하다. 혹시라도 추후 HTML을 수정하고자 한다면 Java소스코드 내에서 수정이 이루어지기 때문에
		...
		*/
		...
		// 3. 생성된 스트림을 토앻 응당 html구문을 한줄씩 출력
		out.println("<html>");
		out.println("<head>");
		out.println("<style>");
