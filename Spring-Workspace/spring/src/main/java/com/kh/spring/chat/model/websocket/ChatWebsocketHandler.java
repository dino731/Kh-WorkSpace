...
public class ChatWebsocketHandler extends TextWebSocketHandler{
	 ...
	 * - 클라이언트로 부터 메세지가 도착했을 시 실행.
	 * 
	 * void afterConnectionEstablished(WebSocketSession session)
	 * - 클라이언트와 연결이 완료되고, 통실할 준비가 되면 실행
	 * 
	 * void afterConnectionClosed(WebSocketSession session, closeStatus closeStatus)
	 * - 클라이언트와 연결이 "종료"되면 실행
	 * 
	 * void handlerTranspoerError(WebSocketSession session, Throwable exception)
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception{
		...
		// 웹소켓이 연결이 종료되는 경우 session내부에 있는 클라이언트의 session정보를 삭제
	...
	public void handlerTextMessage(WebSocketSession session, TextMessage message) throws Exception{
		...
		if(result > 0) {
			...
			for( WebSocketSession s : sessions) {
				...
				// 메세지에 담겨있는 채팅방 번호와 현재 웹소켓 세션에 있는 채팅방 번호를 비교
				if(chatMessage.getChatRoomNo() == chatRoomNo) {
					// 같은 방 사용자라면 클라이언트에게 JSON형식으로 메세지 보내기
