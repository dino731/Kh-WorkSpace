...
$(()=>{
	$("#loginModal").modal()
					.on('hide.bs.modal',e =>{
						// modal을 비활성화시 이전 페이지로 이동한다. (x버튼, 취소, 모달창 이외의 영역 클릭시)
						loccation.href ='${pageContext.request.contextPath}';
					})
})
</script>
				...
				<form:form
					...
					<div class="modal-body">
						<c:if test="${param.error != null }">
							<span class="test-danger">아이디 또는 비밀번호가 일치하지 않습니다..</span>
						</c:if>
					...
					<div class="modal-footer">
						<div>
							...
							<label for="remember-me" class="form-check-label">로그인유저</label>
						</div>
