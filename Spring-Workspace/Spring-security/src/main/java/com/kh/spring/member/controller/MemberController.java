...
public class MemberController {
	...
	public String myPage(Model model,
		...
		//1. SecurityContextHolder로 부터 사용자의 인증 정보 가져오기
	...
	public String updateMember(
		...
		// 1. 업무로직 : DB의 Member테이블에 회원 정보 수정 요청을 보낸 회원의 정보 수정
		
		// 2. 변경된 회원 정보를 DB에서 얻어온 후 Session에 저장하는 대신
		//		SecurityContext의 Authentication 객체를 직접 수정.
		...
		for( GrantedAuthority auth : oldAuthorites) {
			SimpleGrantedAuthority simpleAuth = new SimpleGrantedAuthority(auth.getAuthority()); // 문자열 형태의 권한을 매개변수로 auth객체 생성
		...
		// SecurityContextHolder에 새로운 인증 객체 등록
