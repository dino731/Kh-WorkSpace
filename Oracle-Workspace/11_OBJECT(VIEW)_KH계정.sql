...
SELECT * FROM ( /*(서브쿼리 넣어서 사용 가능)*/ );

-- 위와 같이 복잡한 서브쿼리를 이용하여 데이터들을 조회하는 것 보다
-- 한번 뷰를 생성한 후 뷰의 이름으로 간단하게 조회하는게 효율이 더 좋다

-- 보너스 컬럼이 없는 뷰에서 보너스도 조회하고 싶을 경우 새롭게 뷰를 생성해줘야하는데,
-- 삭제하고 다시 생성하는 것 보다 CREATE OR REPLACE VIEW를 사용하면 좀 더 간편하게 생성 가능
...
/*
    ...
    쿼리문이 TEXT문구로 저장되어 있음
    해당 계정이 가지고 있는 VIEW들에 대한 내용을 조회하고자 한다면 USER_VIEWS
    데이터 딕셔너리를 이용하면 된다.
*/
...
/*
    뷰 컬럼에 별칭 부어
    서브쿼리 부분에 SELECT절에 함수 OR 산술연산식이 기술되어 있는 경우 "반드시" 별칭으로 지정해줘야함
*/
...
-- 또 다른 방법으로 별칭 부여하기 (단,, 모든 칼럼에 대해서 모두 별칭을 기술해야함)
...
FROM VW_EMP_JOB
WHERE 성별 = '여';
-- 뷰에서 생성할때 붙인 별칭, 리터럴 값들을 SELECT시 활용 가능하다.
...
SELECT * FROM USER_VIEWS;
/*
    *생성된 뷰를 이용해서 EML(INSERT, UPDATE, DELETE) 사용가능
    
    주의 사항 : 뷰를 통해서 조작하게 된다면 실제 데이터가 담겨있는 테이블에도 변경사항이 적용된다.
*/
CREATE VIEW VW_JOB
AS SELECT * FROM JOB;
...
SELECT * FROM VW_JOB;
SELECT * FROM JOB;
-- JOB테이블에도 값이 INSERT된 것을 확인 가능하다.
...
/*
    ...
    * 하지만 뷰를 가지고 DML이 불가능한 케이스가 더 많다. => 한번의 처리가 들어간 경우에는 불가능함
    ...
    2. 뷰에 정의되어 있지 않은 컬럼 중에 베이스테이블 상에 NOT NULL 제약 조건이 지정된 경우
    3. 산술연산식 또는 함수를 통해서 정의되어 있는 경우
    4. 그룹함수나 GROUP BY 절이 포함된 경우
    5. DISTINCT 구문이 포함된 경우
    6. JOIN을 이용해서 여러 테이블을 매칭 시켜놓은 경우
*/
...
VALUES (400,'민경민',5000000,60000000); -- 실제 존재하지 않는 테이블에 값을 추가할 수 없음.
...
INSERT INTO VW_GROUPDEPT VALUES('D3',8000000,4000000); -- 안됨.

UPDATE VW_GROUPDEPT
SET 합계 = 8000000
WHERE DEPT_CODE ='D1'; -- 안됨 "data manipulation operation not legal on this view"

DELETE FROM VW_GROUPDEPT
WHERE 합계 = 5210000; -- 안됨 "data manipulation operation not legal on this view"
...
--2. FORCE/NOFORCE 옵션 : 실제 테이블이 없더라도 VIEW를 먼저 생성할 수 있게 해주는 옵션
...
-- SELECT문의 WHERE 절에서 사용한 칼럼을 수정하지 못하게 하는 옵션
