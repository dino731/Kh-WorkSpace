/*
    <트리거>
    내가 지정한 테이블에 INSERT, UPDATE, DELETE 등의 DML문에 의해 변경사항이 생길때(테이블에 이벤트가 발생했을때)
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있는 객체
    
    EX.
    회원 탈퇴시 기존의 회원테이블에 데이터 DELETE후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리해야될 때
    신고 횟수가 일정수를 넘었을때 묵시적으로 해당 회원을 블랙리스트 처리되게끔 한다거나   
    ...
    *트리거 종류
    ...
    SQL문에 의해 영향을 받는 각 행에 따른 분류
        > STAREMENT TRIGGER(문장 트리거) : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
...
-- 2. 상품 입출고 상세 이력 테이블 (TB_PRODERAIL)
...
-- 이력 번호를 자동으로 발생시켜주는 시퀀스 생성(SEQ_DCODE)
...
-- TB_PRODETAIL 테이블에 INSERT 이벤트가 발생한 후
-- TB_PRODUCT 테이블에 매번 자동으로 재고 수량을 변경해주게끔 트리거를 정의하기

/*
    - 상품이 입고된 경우 -> 해당 상품을 찾아서 재고수량 + (증가_ UPDATE
    UPDATE TB_PRODUCT
        SET STOCK = STOCK + 현재 입고된 수량(TB_PRODETAIL 테이블에 INSERT된 AMOUNT값)
...
BEGIN
    ...
    IF(:NEW.STATUS = '입고')
        THEN
        UPDATE TB_PRODUCT
        SET STOCK = STOCK + :NEW.AMOUNT -- :NEW 키워드를 통해 인서트된 값을 활용할 수 있다.
...
/*
    ...
    2. ★ 관리적 측면에서 형상 관리가 불가능하기 때문에 관리가 불편하다. (신입이 봤을때 추적이 불가능하다)
    3. 트리거를 남용하게 되는 경우 예상치 못한 사태가 발생할 수 있으며, 유지보수가 힘들다.
*/
