/*
    *DDL(DATE DEFINISTION LANGUAGE)
    ...
    <테이블 수정>
    [표현법]
    ALTER 객체명(TABLE, INDEX, USER...) 테이블명 수정할 내용;
...
*/
-- 1. 컬렴 추가/ 수정/ 삭제
-- 1_1. 칼럼 추가 (ADD): ADD 추가할컬럼명 자료형 [DEFAULT 기본값] 
...
-- LNAME칼럼 추가 DEFAULT값으로 '한국'이라고 지정하기
ALTER TABLE DEPT_COPY ADD LNAME VARCHAR2(20) DEFAULT '한국';

-- 1_2. 칼럼 수정 (MODIFY)
--      칼럼의 자료형 수정 : MODIFY 수정할 칼럼명 바꾸고자하는 자료형
--      DEFAULT 값 수정시 : MODIFY 수정할 칼럼명 DEFAULT 바꾸고자 하는 기본값

-- DEPT_COPY 테이블의 DEPT_ID의 자료형을 CHAR(3)로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);

-- 현재 변경하고자 하는 컬럼에 이미 담겨있는 값과 완전히 다른 타입으로 변경해보기(불가능X)
--"column to be modified must be empty to change datatype"
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;

--현재 변경하고자 하는 칼럼에 이미 담겨있는 값보다 더 작은 크기고 변경해보기
...
-- 한번에 여러개의 컬럼 변경 가능
-- DEPT_TITLE 칼럼의 데이터 타입을 VARCHAR2(4)으로,
-- LOCATION_ID 컬럼의 데이터 타입을 VARHCAT2(2)로
-- LNAME컬럼의 기본값을 '미국'으로
...
-- 1_3. 컬럼 삭제(DROP COLUMN) : DROP COLUMN 삭제하고자 하는 칼럼명
...
/*
    ...
    - FOREIGN KEY   : ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명 [(참조할칼럼명)];
...
*/
-- DEPT_COPY 테이블로 부터 DCOPY_PK 제약조건 지우기
...
-- 3_1. 칼럼명 변경 : RENAME COLUMN 기존컬렴명 TO 바꿀컬럼명
-- DEPT_COPY 테이블에서 DEPT_TITLE을 DEPT_NAME으로 바꾸기
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

-- 3_2. 제약조건명 변경 : RENAME CONSTRAINT 제약조건명 TO 바꿀 컬럼명
