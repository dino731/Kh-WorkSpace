...
-- 프로시저 생성하기 ( IS가 PL/SQL문의 DECLARE 같은 역할)
...
-- 매개변수 있는 프로시저 만들기
-- IN   : 프로시저 실행시 필요한 값을 받는 변수 (일반적인 매개변수와 동일한 개념)
...
/*
    ...
    EX. DB에서 대용량의 데이터를 SELECT문으로 받아와서 자바에서 처리하는 CASE
        ...
        DB에서 처리하는게 성능이 좋다 (데이터를 넘길때마다 자원이 소비됨)  - 커넥션은 사용할때마다 바로바로 어쩌구 해줘야함
    ...
    정리!!
    한번에 처리되는 데이터량이 많고, 성능을 요구하는 처리는 대체로 자바보다 DB상에서 처리하는 것이 성능 측면에서 나을것이고
    소스관리(유지보수) 측면에서는 자바에서 작업하는게 더 좋다
*/
----------------------------------------------------------------------------------------------------
/*
    <FUNCTION>
    프로시저와 거의 유사하지만 실행 결과를 반환받을 수 있음.
...
-- EMP_ID 를 전달받아서 연봉(보너스 포함 연봉)을 계산해서 출력해주는 함수 만들기.
-- CALC_SALARY
CREATE OR REPLACE FUNCTION CALC_SALARY(V_SALARY NUMBER)
RETURN NUMBER
IS
    RESULT NUMBER;
BEGIN
