...
/*
    CHARATER => NUMBER
    - TO_NUMBER(CHARATER, 포맷) : 문자열 데이터를 숫자형으로 변환 (결과값은 NUMBER)
*/
-- 자동형변환 예시(문자열 -> 숫자)
SELECT '123'+'456'
...
FROM DUAL; -- 문자(,)가 포함되어 있기 때문에 자동형변환이 안됨
...
-- 문자열, 숫자, 날짜 형변환( TO_CHAR, RO_NUMBER, TO_DATE)
-------------------------------------------------------------------------------
...
/*
    ...
    해당컬럼에 값이 존재할 경우 기존의 컬럼값을 반환
    컬럼에 값이 존재하지 않을 경우 내가 제시한 값이 반환된다.
*/
...
*/
-- 사번, 사원명, 주민등록번호, 주민등록번호로 부터 성별 자리를 추출해서 1이면 남자, 2면 여자 성별 칼럼 만들기
SELECT EMP_ID, EMP_NAME, EMP_NO,
DECODE(SUBSTR(EMP_NO,8,1), '1', '남자', 2, '여자') "성별"
FROM EMPLOYEE;

-- 직원들의 급여를 인상시켜서 조회
-- 직급코드가 'J7'인 사원 급여를 10%인상해서 조회
-- 직급코드가 'J6'인 사원 급여를 15%인상해서 조회
-- 직급코드가 'J5'인 사원 급여를 20%인상해서 조회
-- 그 외의 직급코드는 급여를 5%인상해서 조회
-- 사원명, 직급코드, 변경전 급여와 변경 후 급여를 출력
...
/*
    CASE WHEN THEN 구문
    - DECODE 선택 함수와 비교하면 DECODE는 해당 조건 검사시 동등비교만을 수행
...
-- 그룹함수 : 데이터드르이 합(SUM), 데이터들의 평균(AVG)
/*
...
-- 3. MIN(ANY타입) : 해당 칼럼값들 중 가장 작은 값을 반환
-- 전체 사원들 중 최저급여, 가장 작은 이름값, 가장작은 이메일 값, 입사일이 가장 예전인 사원
SELECT 
    MIN(SALARY), MIN(EMP_NAME),MIN(EMAIL), MIN(HIRE_DATE)
FROM EMPLOYEE;

-- 4. MAX(ANY타입컬럼) : 해당컬럼값들중 가장 큰 값을 반환
SELECT 
...
-- COUMT(칼럼이름) : 제시한 해당칼럼의 값이 NULL이 아닌 것만 세서 반환
