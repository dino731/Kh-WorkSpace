...
-- 불충분한 권한 에러 : SAMPLE계정에 테이블을 생성할 수 있는 권한을 부여하지 않음
...
-- 계정이 소유하고 있는 테이블들을 조작하는 것도 가능해짐(DML)
...
CREATE VIEW V_TEST
AS SELECT * FROM TEST;
-- "insufficient privileges" 불충분한 권한 에러.
...
-- DEPARTMENT 테이블에 접근할 수 있는 권한이 없기 떄문에 오류
