/*
    ...
    EX) 주차번호, 회원번호, 사번, 게시글 번호등
    => 순차적으로 겹치지 않는 숫자로 채번할 때 사용할 예정
    
    1. 시퀀스 객체 생성 구문
    
    [표현법]
    CREATE SEQUENCE 시퀀스명
    START WITH 시작 숫자 => 생략 가능, 처음 발생시킬 시작 값의 기본 값은 1
    INCREMENT BY 증가 값 => 생략 가능 기본 값은 1씩 증가, 한번 시퀀스가 증가할 때마다 몇씩 증가할 건지 결정
    MAXVALUE 최대값 => 생략 가능, 최대값 기정
    MINVALUE 최소값 => 생략 가능, 최소값 지정
    CYCLE/NOCYCLE => 생략 가능, 값의 순환 여부를 결정
    CACHE 바이트 크기 / NOCACHE => 생략 가능, 캐시메모리 여부 지정
                                기본 캐시 사이즈는 20BYTE
    * 캐시 메모리
    시퀀스로 부터 미리 발생될 값들을 생성해서 저장해두는 공간
    매번 호출할 때마다 새로이 번호를 생성하는 것보다
    캐시메모리 공간에 미리 생성된 값들을 가져다 쓰게되면 훨씬 속도가 빠름
...
/*
    ...
    시퀀스명.CURRVAL : (커런트약자)현재 시퀀스의 값 (마지막으로 성공적으로 발생된 NEXTVAL의 값)
...
-- 시퀀스가 생선되고 나서 NEXTVAL을 한번이라도 수행하지 않는 이상 CURRVAL을 수행할 수 없기 때문에 발생된 에러
-- CURRVAL은 마지막으로 성공적으로 수행된 NEXTVAL의 값을 저장해서 보여주는 임시 값이기 때문.
...
/*
    ...
    CACHE 바이트 크기 / NOCACHE => 생략 가능, 캐시메모리 여부 지정
                                기본 캐시 사이즈는 20BYTE
    => START WITH는 변경 불가 : 정 바꾸고 싶다면 시퀀스를 삭제했다가 재생성 해줘야함.
*/
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, SAL_LEVEL, HIRE_DATE)
VALUES (SEQ_EID.NEXTVAL, '고길동','123456-1234567','J2','S3',SYSDATE);

SELECT * FROM EMPLOYEE;

-- 시퀀스가 가장 많이 사용되는 위치는 INSERT문의 PK값에 넣을 때
